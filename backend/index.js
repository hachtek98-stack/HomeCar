  ...
  db.get(
    ...
    (err, row) => {
      ...
      if (row) {
        ...
        const name =
          role === "patient" ? "Patient " + phone : "Infirmier " + phone;
        db.run(
...
app.post("/api/requests", (req, res) => {
  ...
  if (!patientId || !prescriptionImage) {
    return res
      .status(400)
      .json({ error: "patientId and prescriptionImage are required" });
  }
  ...
  db.run(
    ...
    function (err) {
      ...
      if (this.changes === 0)
        return res
          .status(404)
          .json({ error: "Request not found or already paid" });

      // Notify nurses about the new paid request
      db.all(
        `SELECT pushToken FROM users WHERE role = 'nurse' AND pushToken IS NOT NULL`,
        [],
        (err, rows) => {
          if (!err && rows.length > 0) {
            ...
            sendPushNotification(
              tokens,
              "Nouvelle demande de prélèvement disponible !",
            );
          }
        },
      );
      ...
      db.get(
        ...
        (err, row) => {
          if (!err && row && row.pushToken) {
            sendPushNotification(
              [row.pushToken],
              "Un infirmier a accepté votre demande et est en route !",
            );
          }
        },
      );
