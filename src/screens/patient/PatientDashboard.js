...
// ⚡ Bolt: Extract stateless utility function outside of component to avoid recreation
...
export default function PatientDashboard() {
  ...
  // ⚡ Bolt: Memoize derived list to maintain reference equality for FlatList optimization
  // Filter requests for the current patient
  const patientRequests = useMemo(() => {
    return requests.filter(req => req.patientId === user?.id);
  }, [requests, user?.id]);

  // ⚡ Bolt: Memoize renderItem to prevent FlatList from unnecessarily re-rendering items
