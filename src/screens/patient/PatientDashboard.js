...
export default function PatientDashboard() {
  ...
  // Filter requests for the current patient and memoize
  // ⚡ Bolt: wrap patientRequests in useMemo to prevent recalculating on every render, thus keeping reference equality for FlatList
  const patientRequests = useMemo(() => {
    return requests.filter(req => req.patientId === user.id);
  }, [requests, user.id]);

  // ⚡ Bolt: wrap renderItem in useCallback to avoid recreating function reference on every render
