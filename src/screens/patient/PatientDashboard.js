...
// ⚡ Bolt: Move stateless utility functions outside the component scope to avoid recreation on every render
...
export default function PatientDashboard() {
  ...
  // ⚡ Bolt: Memoize the filtered array to preserve reference equality and prevent unnecessary FlatList re-renders
  const patientRequests = useMemo(() =>
    requests.filter(req => req.patientId === user.id),
  [requests, user.id]);

  // ⚡ Bolt: Memoize renderItem to prevent inline function recreation on every render, allowing FlatList optimizations
