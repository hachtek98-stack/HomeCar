...
// ⚡ Bolt: Extract stateless utility function out of the component scope
...
export default function PatientDashboard() {
  ...
  // ⚡ Bolt: Memoize filtered requests to preserve reference equality and prevent FlatList re-renders
  // Filter requests for the current patient
  const patientRequests = useMemo(() => {
    return requests.filter(req => req.patientId === user.id);
  }, [requests, user.id]);

  // ⚡ Bolt: Memoize renderItem to prevent recreation on every render
