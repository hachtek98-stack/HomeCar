...
export default function PatientDashboard() {
  ...
  // Filter requests for the current patient, memoized to prevent re-creation on every render
  const patientRequests = useMemo(() => {
    return requests.filter(req => req.patientId === user.id);
  }, [requests, user.id]);

  // Memoize renderItem to prevent FlatList from re-rendering all items unnecessarily
