...
// ⚡ Bolt: Extract stateless utility function outside component scope
...
export default function PatientDashboard() {
  ...
  // ⚡ Bolt: Memoize derived array to preserve referential equality for FlatList's internal PureComponent
  // Filter requests for the current patient
  const patientRequests = useMemo(() => {
    return requests.filter(req => req.patientId === user?.id);
  }, [requests, user?.id]);

  // ⚡ Bolt: Memoize renderItem to prevent re-creation on every render
